...
function EduSection(props) {
  ...
  return (
    <div>
      <div
        className="super-box"
        onClick={() => props.redirectToVideos(props.title)}
      >
...
export default function Education() {
  ...
  const updatePageHtml = async () => {
    try {
      // const user = await JSON.parse(localStorage.getItem(import.meta.env.CODETOGETHER_APP_LOCALHOST_KEY));
      const EducationAPIresponse = await axios.post(
        import.meta.env.VITE_SERVER_BASE_URL + "/education",
        { cfID: user.cfID },
        { withCredentials: true }
      );
      const EducationInfo = EducationAPIresponse.data.data;
      const EducationComponent = EducationInfo.map((category, index) => (
        <EduSection
          key={index}
          title={category.title}
          _id={category._id}
          redirectToVideos={redirectToVideos}
        />
      ));
