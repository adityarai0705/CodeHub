  ...
  const updatePageHtml = async () => {
    try {
      ...
      const user = await JSON.parse(
        localStorage.getItem(import.meta.env.CODETOGETHER_APP_LOCALHOST_KEY)
      );
      const VideoAPIresponse = await axios.post(
        import.meta.env.VITE_SERVER_PATH + "/education/videos",
        { categoryID: educationCategory, cfID: user.cfID },
        ...
        <>
          <div>
            ...
            >
              ...
              <NavSpace />
              <div className="videoList-heading">
                <small>Education</small>/{educationCategory}
              </div>
              <div className="videoContainer">{VideoLists}</div>
            </div>
            <Footer />
          </div>
        </>
